...
``recent`` are real projections over the runs ledger; ``fleet`` and
``system-health`` aggregate Monitor/System sources that have no live producer yet,
so they ship as typed-empty stubs (``data_source="pending_m4"``).
"""
...
def recent(runs_path: Path = Depends(get_runs_path)) -> Envelope:
    ...
    return ok(items, meta={"ttl": 300})


@router.get("/fleet", response_model=Envelope)
def fleet() -> Envelope:
    return ok([], meta={"data_source": "pending_m4", "ttl": 60})
