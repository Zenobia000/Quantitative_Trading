...
from quant_platform.services.research_validation.workflows.doe import DOEResult, run_doe
...
def test_run_doe_does_not_import_backtest_directly():
    ...
    src = pathlib.Path(
        "src/quant_platform/services/research_validation/workflows/doe.py"
    ).read_text()
    tree = ast.parse(src)
