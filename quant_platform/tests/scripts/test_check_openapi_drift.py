...
# W-carve: pyproject lifted to quant_platform/ and tests/ moved up one level, so
# the repo root (where scripts/ lives) is parents[3]; the backend root is parents[2].
...
def test_datasource_members_parses_live_enum():
    envelope = (_BACKEND_ROOT / "src" / "quant_platform" / "apps" / "api" / "envelope.py")
