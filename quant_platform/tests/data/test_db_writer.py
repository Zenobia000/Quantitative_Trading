...
from quant_platform.packages.infrastructure.db_kernel import DBConfig
from quant_platform.services.data_platform.bundle_writer import _upsert_frame
...
def test_connection_refuses_default_password(monkeypatch) -> None:
    ...
    from quant_platform.packages.infrastructure.config.settings import INSECURE_POSTGRES_PASSWORD
    from quant_platform.packages.infrastructure.db_kernel import _connection
...
def test_upsert_runs_calls_execute_values_with_run_id_conflict() -> None:
    ...
    with patch(
        "quant_platform.packages.infrastructure.runs_writer._connection"
...
def test_upsert_runs_empty_returns_zero() -> None:
    ...
    with patch("quant_platform.packages.infrastructure.runs_writer._connection") as mock_conn_ctx:
        n = upsert_runs([])
    assert n == 0
...
def test_real_upsert_idempotent() -> None:
    ...
    from quant_platform.services.data_platform.bundle_writer import upsert_bundle
...
def _capture_sql(fn, rows):
    ...
    conn = MagicMock()
    conn.cursor.return_value.__enter__.return_value = cur
    with patch("quant_platform.services.monitoring_ops.telemetry_writer._connection") as mock_ctx, patch(
        "psycopg2.extras.execute_values"
    ) as mock_exec:
...
def test_trade_log_writers_empty_returns_zero_without_connection() -> None:
    ...
    for fn in (upsert_signals, upsert_equity_snapshots, upsert_fills):
        with patch("quant_platform.services.monitoring_ops.telemetry_writer._connection") as mock_ctx:
            assert fn([]) == 0
            mock_ctx.assert_not_called()


def test_upsert_signals_inserts_with_jsonb_reason() -> None:
    from quant_platform.services.monitoring_ops.telemetry_writer import upsert_signals
...
def test_upsert_equity_snapshots_upserts_on_pk() -> None:
    from quant_platform.services.monitoring_ops.telemetry_writer import upsert_equity_snapshots
...
def test_upsert_fills_single_store_with_strategy_id() -> None:
    ...
    from quant_platform.services.monitoring_ops.telemetry_writer import upsert_fills
