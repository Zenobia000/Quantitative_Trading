...
def test_ingest_single_stock_falls_back_to_fetch_on_miss(tmp_path):
    ...
    with patch.object(ParquetCache, "load_or_none", return_value=None), patch(
        "quant_platform.services.data_platform.finmind_etl.write_parquet"
    ) as mock_write:
        result = cached_or_fetch(
...
def test_ingest_universe_returns_dict_and_failed_list(tmp_path):
    ...
    from quant_platform.services.data_platform.finmind_bundle import ingest_universe
    ...
    with patch(
        "quant_platform.services.data_platform.finmind_bundle.cached_or_fetch",
