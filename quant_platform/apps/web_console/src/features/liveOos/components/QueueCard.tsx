  ...
  return (
    <section className="border border-border bg-surface">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2">
        <h2 className="font-mono text-[13px] font-semibold text-text">{item.strategy}</h2>
      ...
      {/* selection audit */}
      <div className="border-b border-border/60 bg-base px-3 py-2 text-xs">
      ...
      <div className="px-3 py-2">
        {isBerthKind(obs.kind) ? (
          ...
          <p className="text-xs text-text-muted">{t('queue.replayPending')}</p>
        )}
      </div>

      {/* three-way links: report / candidate / strategy asset */}
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border/50 px-3 py-2 text-xs">
        {runId ? (
