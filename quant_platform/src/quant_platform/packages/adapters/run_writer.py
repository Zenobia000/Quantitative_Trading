...
from quant_platform.packages.adapters.run_db_mapper import (
    config_to_status_row,
    run_record_to_db_row,
)
from quant_platform.packages.adapters.runs_store import DEFAULT_RUNS_PATH, append_run
...
def mark_run_status(cfg: Any, status: str, *, writer: Any = None) -> bool:
    ...
    try:
        if writer is None:
            from quant_platform.packages.infrastructure import runs_writer as writer
