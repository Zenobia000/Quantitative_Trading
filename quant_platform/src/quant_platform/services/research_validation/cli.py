"""Research-loop CLI — formalizes the one-off-script workflow.
    ...
    uv run python -m quant_platform.services.research_validation.cli run-is \\
    ...
    uv run python -m quant_platform.services.research_validation.cli validate-strategy momentum
...
from quant_platform.packages.application.is_harness import run_and_judge_with_returns
...
from quant_platform.packages.adapters.runs_store import DEFAULT_RUNS_PATH, read_runs
...
from quant_platform.services.research_validation.validation.tearsheet import write_tearsheet
...
def _gate_for_record(rec: dict):
    ...
    """
    from quant_platform.services.research_validation import runners as _runners  # noqa: F401 — register
    from quant_platform.services.research_validation.strategies.protocol import get_strategy_gate
...
def portfolio_check_cmd(candidate, fleet, n_trials, oos_sharpe, series_dir) -> None:
    ...
    from quant_platform.packages.adapters.run_series_store import read_series
    from quant_platform.services.research_validation.validation.portfolio_gate import portfolio_gate_report
...
def validate_strategy_cmd(name: str | None, list_all: bool) -> None:
    ...
    """
    from quant_platform.services.research_validation import runners as _runners  # noqa: F401
    from quant_platform.services.research_validation.strategies.conformance import check_strategy
    from quant_platform.services.research_validation.strategies.protocol import list_strategies
...
def compare_cmd(runs_path, baseline) -> None:
    ...
    from quant_platform.packages.domain.compare import compare_runs
...
def sweep_cmd(strategy, base_params, grid, stocks, start, end, out_csv) -> None:
    ...
    from quant_platform.packages.application.sweep import expand_grid, run_sweep
    from quant_platform.services.research_validation.strategies.protocol import get_strategy
...
def _override_window(cfg, is_start: str | None, is_end: str | None):
    ...
    from quant_platform.services.research_validation.workflows.config import revalidate_with_overrides
...
def doe_cmd(strategy, dry_run, is_start, is_end, out_csv) -> None:
    ...
    from quant_platform.services.research_validation.workflows.loader import get_doe_config
    try:
...
def go_gates_cmd(strategy, dry_run) -> None:
    ...
    from quant_platform.services.research_validation.workflows.loader import get_go_gates_config
    try:
...
def truth_gate_cmd(strategy, dry_run) -> None:
    ...
    from quant_platform.services.research_validation.workflows.loader import get_truth_gate_config
    try:
...
def build_universe_cmd(strategy, dry_run) -> None:
    ...
    from quant_platform.services.research_validation.workflows.loader import get_universe_config
    try:
    ...
    if dry_run:
        ...
        return
    from quant_platform.services.research_validation.workflows.universe import run_build_universe
...
def paper_replay_cmd(strategy, dry_run) -> None:
    ...
    from quant_platform.services.research_validation.workflows.loader import get_paper_replay_config
    try:
    ...
    if dry_run:
        ...
        return
    from quant_platform.packages.application.is_harness import load_merged_parquet
    from quant_platform.services.research_validation.workflows.paper_replay import run_paper_replay_workflow
...
def evaluate_cmd(strategy, profile, data_dir, symbols, start, end, hypothesis, no_ingest) -> None:
    ...
    from quant_platform.services.research_validation.evaluation import evaluate
...
def candidates_list_cmd(state, strategy) -> None:
    ...
    from quant_platform.packages.adapters.candidate_store import list_candidates
...
def candidates_decide_cmd(candidate, action, label, reason) -> None:
    ...
    from quant_platform.packages.domain.candidate_state import IllegalTransitionError
    from quant_platform.packages.adapters.candidate_store import (
...
def branches_create_cmd(parent_evaluation_id, sets, origin, profile, note) -> None:
    ...
    from quant_platform.packages.adapters import branch_store
...
def branches_list_cmd(strategy, parent_evaluation_id) -> None:
    ...
    from quant_platform.packages.adapters.branch_store import list_branches
