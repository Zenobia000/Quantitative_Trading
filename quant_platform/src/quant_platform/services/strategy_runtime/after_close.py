...
from quant_platform.packages.domain.market_data_errors import NoMarketDataError
...
def default_watch_status(strategy: str, as_of: date) -> Any:
    ...
    from quant_platform.services.governance_release.watch_registry import status
...
def default_expire_watches(as_of: date) -> list[str]:
    ...
    from quant_platform.services.governance_release.watch_registry import expire_due
...
def _watch_line(watch: Any) -> str:
    ...
    if watch is None:
        return ""
    from quant_platform.services.governance_release.watch_registry import NOMINAL_TRADING_DAYS
...
def safe_discord_notify(message: str, ok: bool) -> None:
    ...
    """
    try:
        from quant_platform.services.monitoring_ops.discord_notifier import notify_error, notify_info
...
def _default_state_loader(strategy: str) -> Any:
    ...
    from quant_platform.services.data_platform.db_reader import load_broker_state
...
def build_session_runner(
    ...
    *,
    ...
    from quant_platform.services.strategy_runtime.live_session import (
