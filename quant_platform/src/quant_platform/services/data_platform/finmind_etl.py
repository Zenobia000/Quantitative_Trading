...
CLI usage::
    ...
    python -m quant_platform.services.data_platform.finmind_etl \\
...
from quant_platform.packages.infrastructure.config.settings import get_settings

# write_parquet moved to data.parquet_writer (W5.2c-2, DB-free path); re-exported
# here for the CLI main() and existing ``from data.finmind_etl import write_parquet``
# consumers (parquet_cache, tests). The research-reachable edge (finlab_source)
# imports parquet_writer directly, severing research→finmind_etl→db_writer.
from quant_platform.packages.infrastructure.parquet_writer import write_parquet
...
def fetch_bundle(
    ...
    """
    from quant_platform.services.data_platform.adjustment import (
