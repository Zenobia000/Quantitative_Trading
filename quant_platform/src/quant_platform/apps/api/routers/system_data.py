...
from quant_platform.apps.api.deps import get_data_root
...
def bundles(
    ...
    never 500."""
    from quant_platform.services.data_platform.bundle_registry import scan_bundles
...
def bundle_quality(bundle_id: str, data_root: Path = Depends(get_data_root)) -> Envelope:
    ...
    from quant_platform.services.data_platform.bundle_registry import compute_bundle_quality
...
def universes(data_root: Path = Depends(get_data_root)) -> Envelope:
    ...
    from quant_platform.services.data_platform.universe_registry import list_universes
...
def datasets(
    ...
    monitor."""
    from quant_platform.services.data_platform.dataset_presence import (
        presence_for_category,
        table_for_category,
    )
    from quant_platform.services.data_platform.finlab_catalog import CATALOG_VERSION, load_catalog
    from quant_platform.services.data_platform.strategy_data_index import (
...
def ingest(req: IngestRequest, data_root: Path = Depends(get_data_root)) -> Envelope:
    ...
    from quant_platform.services.data_platform.universe_registry import symbols_for
    from quant_platform.services.execution_gateway.collaborators import make_ingest
...
def universe_build(req: UniverseBuildRequest) -> Envelope:
    ...
    FinLab / the network."""
    from quant_platform.services.research_validation.workflows import universe as _universe
