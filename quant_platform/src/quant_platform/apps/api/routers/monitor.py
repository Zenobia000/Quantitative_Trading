...
from quant_platform.apps.api.deps import get_telemetry_reader
...
def _registered_strategies() -> list[str]:
    ...
    for mod in (
        "quant_platform.services.research_validation.strategies.inst_flow.runner",
        "quant_platform.services.research_validation.strategies.momentum.runner",
        "quant_platform.services.research_validation.strategies.four_layer_resonance.runner",
    ):
        try:
            __import__(mod)
        except Exception:
