...
- **The strategy holds an active觀察艙 berth** (ADR-033): a real after-close run is
  ...
  **primary** way a berth comes to exist is the **live-OOS selection queue**: an
  ...
  uv run python -m quant_platform.services.strategy_runtime.cli live-oos consume   # what ExecStartPre runs
  uv run python -m quant_platform.services.strategy_runtime.cli live-oos list      # inspect the queue

  # Manual override (ops / testing) — enroll a berth directly:
  uv run python -m quant_platform.services.strategy_runtime.cli \
      watch enroll --strategy inst_flow --dsr 0.908
  uv run python -m quant_platform.services.strategy_runtime.cli watch status
  ```
...
`uv run python -m quant_platform.services.research_validation.cli truth-gate --strategy <s> --dry-run` smoke.
