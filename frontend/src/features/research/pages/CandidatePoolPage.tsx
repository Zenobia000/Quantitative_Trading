...
import type { Candidate, CandidateState } from '../api/candidates'
...
export function CandidatePoolPage() {
  ...
  const onAction = (candidate: Candidate, action: CandidateAction) => {
    if (reasonRequired(candidate, action)) setPending({ candidate, action })
    else applyLocal(candidate, action)
  }
    ...
    <div>
      ...
      {pending && (
        <ReasonDialog
          ...
          onSubmit={(reason) => applyLocal(pending.candidate, pending.action, reason)}
