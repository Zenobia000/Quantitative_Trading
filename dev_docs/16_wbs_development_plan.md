...
| 8.G 研究工作流（後端+前端）| 94h | 73h | 78% | ✅ gate_state/gate_machine + trials→DSR + CLI 全套 + runs DDL（8.G.1）+ research 前端 9 頁 shipped（含 Promote/TradeReview）+ **8.G.9 兩段閘 two_stage_gate（ADR-025）✅**；剩 8.G.6 token 擴充 + Cmd-K + 8.G.10 配置閘 sizing 收尾。**✅ gate-state 權威已統一（refactor/gate-state-authority，2026-06-10）：`gate_machine` 集中單一 IS 推導（`derive_is_state`/`_is_state_from_result`）+ 詞彙橋（`IS_VERDICT_TO_STATUS` 寫 / `coerce_gate_state` 讀，含 round-trip 不漂移測），cli `_resolve_validation_status` 改用 `coerce_gate_state` 不再靜默忽略持久化 status，修 promote-check 讀小寫 verdict bug**|
