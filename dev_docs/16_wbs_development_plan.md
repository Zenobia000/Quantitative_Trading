...
| 3.0 資料層 | 120h | 72h | 60% | M1 完成；**FinLab 主源落地（`finlab_source.py`：批次 wide→FinMind-parity parquet + survivorship-clean universe builder，ADR-006 realized；FinMind fallback）**；Live feed 待寫 |
...
| 7.0 Paper+實盤 | 110h | 60h | 55% | ✅ v0.5 Wave D（PaperBroker + Risk Gate 12 條 + 3 級熔斷，PR #38）+ 7.D orchestration 引擎/CLI + 7.D.3 端到端 chain test（真實 RiskGate+PaperBroker）+ **7.A.2 trade-log DB writers + 7.D real collaborators 落地**（PR #101/#102：`db_writer` 4 writer + `collaborators.py` production factory + `make_db_sink` 真持久化）；7.B Shioaji / `signal_fn` edge-gated seam ⏳ |
