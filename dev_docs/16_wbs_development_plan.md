...
| **合計** | **1126h** | **909h** | **80%** | M1 ✅ + v0.2–v0.8 後端 waves ✅ + **M3 研究迴圈後端**（PR #76-#78）+ **模組 6.0 統計驗證收尾（6.2.2/6.5.x 結，DOE 方法學執行到結構性定論：4 family 同 ~0.9 Sharpe 牆、無可部署 edge）** + **8.H.1 `Envelope[T]` typed responses（PR #98-100）+ 7.A.2/7.D paper-trade 持久化 & real collaborators（PR #101/#102）**。剩餘缺口主要為 needs-data/實盤（7.B/8.H.8）+ 前端重頁（edge-gated）+ 策略 GO（外部約束 gated）|
...
| 開放 P0 Bug | 0 | 0 |
| 技術債項目 | 4（見下） | < 3 |
| ADR 數量 | 23（+021 REST 契約合一 / 022 多策略艦隊 lite / **023 動能 NO-GO 守 ADR-016 門檻 + 四層廢止**） | 持續 |
