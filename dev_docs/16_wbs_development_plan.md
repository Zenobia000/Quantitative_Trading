...
> **版本：** v3.27 | **更新：** 2026-07-02 | **狀態：** M1 ✅ + **M3 研究迴圈後端 ✅ + 模組 6.0 統計驗證收尾**（統計驗證 / 研究迴圈 + gate machine / risk + 監控 + paper broker / REST API / orchestration / Grafana / momentum / inst-flow / multi-factor / long-short；整體 **86%**、**1053 test pass** / 5 skip / **~92.9%** + FE 44 vitest）。
> **v3.27 survivorship universe 工作流平台化（2026-07-02，[ADR-032](./adrs/ADR-032-survivorship-universe-workflow.md)，sub-project ② 落地）**：ADR-029 刻意延後的 `inst_flow_revalidate_finlab` 從 git 歷史重建為平台工作流 `research/workflows/universe.py::run_build_universe`（fetch FinLab 寬表 → `select_survivorship_universe` 季度 rebalance → `ingest_universe_finlab` → 寫 `universe_manifest.json` provenance；finlab 僅 `getter=None` 時 lazy import，頂層零依賴）。`UniverseConfig` 宣告 + `get_universe_config` + CLI `build-universe --dry-run` + HTTP `_WORKFLOWS` 加 `build_universe`；`TruthGateConfig.parquet_dir` + `run_truth_gate(loader=None)` 讓資料快取覆蓋生效。**inst_flow `TRUTH_GATE` 宣告跟著 cache 走**（`cached_universe_symbols` 掃到料 → survivorship_clean=True + 乾淨 universe + 2010/2024 窗口；缺席 → survivor-only `_WIDE` fallback，反自欺 ADR-030 原則）。**本次只落地工作流與宣告接線，未執行真實重驗**（worktree 無 FinLab 資料 / CI 無 finlab 依賴）——真實 2010→2024 全史 ingest + 重驗須在有 `FINLAB_API_TOKEN` 環境跑 `build-universe` → `truth-gate`，在此之前 inst_flow 續 REJECTED（fallback 態）、paper-ready 續 gated。1116 pass / coverage 92.6%。
> **v3.26 inst_flow 重驗（2026-07-02，ADR-030 後續）**：修正後審判庭重跑 `truth-gate --strategy inst_flow` → **REJECTED**（DSR 0.789 <0.95、WFA OOS+ 33%<60%、survivorship hard-fail——現行 `_WIDE` 係 ADR-024 判定假陽性的 40 檔存活股）。既往「TRUTH GATE REAL」（2026-06-15，FinLab survivorship-clean universe）**在現行程式路徑無法重現**；inst_flow paper-ready 地位**暫停**，gated 於 sub-project ②（FinLab survivorship universe 建構器自 git 歷史重建為平台工作流）+ 重驗過閘。詳見 [inst_flow_truth_gate_revalidation_2026-07-02.md](./inst_flow_truth_gate_revalidation_2026-07-02.md)
...
> **v3.20 策略派發契約（[ADR-028](./adrs/ADR-028-strategy-dispatch-contract.md)，2026-06-16）**：**preset 全移除 + 策略自描述 + conformance gate + HTTP 派發**。① RunConfig →+；dispatch  改走  ——任何已註冊策略可由 HTTP API / CLI 啟動回測。②  Protocol 新增 / ClassVar； 讓 registry 自描述（feeds ）。③ 架構稽核發現 18 個違規（V1-V4）全修： 靜默覆蓋 × 4、ClassVar 缺失 × 8、空結果 metrics 不完整 × 4、 缺失 × 2。④  conformance gate + （全欄位合成資料）；parametrized CI 自動覆蓋所有已註冊策略（4/4 PASS）； CLI。⑤  從  搬入 （D6，de-privilege four_layer）。⑥  刪除 →  取代。987 pass / 0 fail（skipped = DB/zipline integration）。前端 preset 頁面接受暫壞（待 sub-project ③）。
> **v3.19 重構 Stage 3（[ADR-027](./adrs/ADR-027-strategy-contract-and-registry.md) Stage 3，2026-06-16）**：局部清理（純內部重構，無契約/schema 變更）。① `db_writer`：兩個重複的 `_cell` 巢狀序列化函式抽成 module-level `_serialize_cell`（JSONB 欄位 `json.dumps` 單一來源）；② `api/routers/monitor.py`：5 個 `except Exception` 靜默吞錯加 `logger.warning`（真錯誤可見；**保留** ADR-021 pending-stub 降級契約，不改 status code）；③ **`paper_broker` 13 層巢狀 = 假陽性**（驗證為 docstring 文字對齊非程式碼縮排，`_compute_heat` 實際最深 3 層）→ 不動。1000 pass / coverage 94.22%。
> **v3.24 研究流程標準化（sub-project ①.5，[ADR-029](./adrs/ADR-029-research-workflow-standardization.md)，2026-06-17）**：7 支 `scripts/inst_flow_*.py` 一次性腳本（既是 workflow 又是 strategy-specific runner、繞過 ADR-028 dispatch）刪除，邏輯拆成兩層：① 平台通用工作流 `research/workflows/`（`doe`/`go_gates`/`truth_gate`/`paper_replay`，全走 `get_strategy(name).run()`，AST 測試守 dispatch invariant）；② 策略宣告 `strategies/<name>/research_config.py`（`DOE`/`GO_GATES`/`TRUTH_GATE`/`PAPER_REPLAY` frozen Pydantic）。name→package 由 registry 解析（不假設 name==目錄）。CLI 加 `doe`/`go-gates`/`truth-gate`/`paper-replay`（`--dry-run`）；HTTP 加 `POST /research/workflows/{workflow}`（非同步 job）+ `GET /research/workflows/{strategy}`。**新增一隻策略只要寫 `research_config.py` 即可參與所有工作流，零新腳本**。`inst_flow_revalidate_finlab`（FinLab survivorship 建構）+ `daemon_replay`（forward-live 排程）特有邏輯刻意延後至 sub-project ②/8.H.8（git 歷史可復原）。1053 pass / coverage 92.9%。
> **v3.6 新增（ADR-025，2026-06-14 決策）**：驗證機制設計修正 — **驗證閘從 binary 絕對通關改為兩段式（[ADR-025](./adrs/ADR-025-two-stage-validation-gate-and-paper-promotion.md)）**：真偽閘（PBO/DSR/WFA/survivorship-clean，hard-fail）+ 配置閘（Sharpe/相關性/容量 → 倉位，連續；絕對 CAGR 降為參考）+ **paper 前移**（過真偽閘即進 paper 收 live OOS，不再排在絕對門檻後）。修正三缺陷：部署閘≠研究迭代閘被混用造成「專案 blocked」框架錯誤、絕對 CAGR 與市場中性策略錯配、gate 排序死鎖（沒 edge 不准 paper／不 paper 拿不到 live OOS）。**不翻案 ADR-023/024**（動能/多因子/long-short 死於真偽閘 PBO 43-77%，NO-GO 不變）；真正解鎖＝資金流 fixed-config（survivorship-clean WFA OOS 1.30、pre-registered）成為**首個可進 paper 的候選**。前端（v0.4）/研究迭代脫離 binary edge-gate。
...
| ⏸️ Sprint 4 — v0.1 ① **HOLD（ADR-023）**：M0 v3 進場假設設計與實作 | 6/16-6/29 | **⏸️ HOLD：四層廢止 + 動能 NO-GO（ADR-023）→ v0.1「v3 進場重設」前提失效、整支取消，釋放 ~60h 轉「下一候選掃描」（大型股動能/多因子/換 edge）。** 〔原計畫留存〕v0.1 essential ①（修正後，承 v3 IS RED + 顧問收斂）：**Step 1 補 config 注入旁路（STRATEGY_PRESET env + backtest-run --config）+ 算法 v3 wiring（consec_structure/prev_box_upper/bars_since_exit 接進 EvaluateBar）→ 用真 zipline 引擎複核 v3 RED 判決**（校準兩個真相源：zipline vs offline sim；algo:78 原寫死 StrategyConfig()，v3 從未進引擎）。**Step 2 gate_state.py 最小版**（ADR-016 K1/K2/K3 + struct1<30%/churn<20% 純函式 dict，逐條 PASS/FAIL）。 | 4.0 v3進場 + 5.A.6（雙窗口重跑） |
