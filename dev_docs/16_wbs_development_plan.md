...
| 8.H.3 | **M3.2 暴露已算 series**：`/runs/{id}/equity`·`/runs/{id}/trades`·compare `?run_ids`（切 `run_and_judge_with_returns` 持久化） | 6h | 🟡 | 2026-06-07 | **equity/trades ✅**：per-run sidecar（`run_series_store`）+ `run_and_judge_persist` executor 寫入 + `runs_series.py` 讀（12 tests）；解鎖 run_04/05、mon_a 回測半。**compare `?run_ids` 子集模式 ✅（2026-06-10：dedupe+保序子集，FE 多選比較接通）→ 8.H.3 完成** |
