...
| **7.A.4** | **paper 前移：接「過真偽閘」候選收 live OOS（[ADR-025](./adrs/ADR-025-two-stage-validation-gate-and-paper-promotion.md)）** | — | (時間) | ⏳ | — | **候選已定＝資金流 fixed-config**：`inst_flow_truth_gate.py` 實判 **TRUTH GATE REAL**（WFA OOS>0 83% / DSR 0.982 / K3 0.90，目標 25%；`inst_flow_truth_gate_result_2026-06-14.md`）✅。剩：接 7.D real collaborators（7.D.3 已備）進 paper daemon 收 live forward OOS（needs daemon + live 資料 + **真實日曆時間**）。paper 摩擦回饋配置閘（8.G.10）|
