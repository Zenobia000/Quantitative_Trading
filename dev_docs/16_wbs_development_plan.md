...
| **合計** | **1144h** | **919h** | **80%** | M1 ✅ + v0.2–v0.8 後端 waves ✅ + **M3 研究迴圈後端**（PR #76-#78）+ **模組 6.0 統計驗證收尾（6.2.2/6.5.x 結，DOE 方法學執行到結構性定論：4 family 同 ~0.9 Sharpe 牆、無可部署 edge）** + **8.H.1 `Envelope[T]` typed responses（PR #98-100）+ 7.A.2/7.D paper-trade 持久化 & real collaborators（PR #101/#102）** + **8.G.9 兩段閘 two_stage_gate ✅（ADR-025）**。剩餘缺口主要為 needs-data/實盤（7.B/8.H.8）+ **8.G.10 配置閘收尾 + paper 前移（7.A.4，needs-data）** + 前端重頁（脫離 binary edge-gate，ADR-025）+ 策略 GO（外部約束 gated）|
