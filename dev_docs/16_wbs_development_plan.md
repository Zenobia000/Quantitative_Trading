...
> **當前狀態**：整體約 86%（990/1144h）、**1126 tests pass / coverage ~92.6%**、CI 三 job 上線（後端 pytest+coverage gate / 前端 vitest / OpenAPI+DDL drift hard gate）。審判庭 truth gate 五缺陷已修（[ADR-030](./adrs/ADR-030-truth-gate-judgement-fix.md)）；模擬器成本吞噬 bug 已修（PR #148：rebalance 段重疊去重吞掉 lump 成本 → `trim_overlap`，panel 策略歷史數字曾屬免成本高估）。**inst_flow survivorship-clean 平台化重驗（ADR-032 工作流 + 423 檔含下市 + 真實成本）最終判決 REJECTED**（DSR 0.908 < 0.95；WFA OOS+ 100% / holdout 0.892 / K3 0.846 皆過——死於通縮顯著性，詳 [inst_flow_truth_gate_verdicts](./inst_flow_truth_gate_verdicts.md)）；paper-ready 撤回。邊緣候選的處置已由 **[ADR-033](./adrs/ADR-033-paper-watch-tier.md)** 拍板：真偽閘新增零資本 `PAPER_WATCH` 觀察艙態（DSR ∈ [0.90, 0.95) 且 hard-fail 全過 → 倉位恆 0.0、收 3 個月 live OOS；晉升仍需 DSR ≥ 0.95，不放寬部署門檻），inst_flow 依此具觀察艙資格。研究工作流 doe/go_gates/truth_gate/paper_replay/build_universe 全走 ADR-028 dispatch，新策略只寫 `research_config.py`。**after-close 排程器落地（cron/systemd timer 級，CLI `orchestration.cli after-close`：交易日 / 收盤後 / 冪等三守門 + Discord 成敗告警，`deploy/` 附 systemd + cron 範例）——收 live OOS 最後一塊部署層已補齊。** ADR 至 033。
>
