...
| 版本 | 日期 | 變更 |
|:--|:--|:--|
| v3.24 | 2026-06-16 | **研究工作流標準化（[ADR-029](./adrs/ADR-029-research-workflow-standardization.md)，sub-project ①.5）**：`backtest_platform/scripts/`（7 支 inst_flow_*.py）全數刪除；`research/workflows/`（DOE/GO-gates/truth-gate/paper-replay）通用工作流 4 模組，全透過 ADR-028 `get_strategy().run()` dispatch；`strategies/<name>/research_config.py` per-strategy 宣告（inst_flow/momentum/_template/four_layer）；CLI 4 命令（`doe`/`go-gates`/`truth-gate`/`paper-replay`，`--dry-run` 旗標）；`POST /research/workflows/{workflow}` + `GET /research/workflows/{strategy}` HTTP API。任何已註冊策略新增 `research_config.py` 即加入所有工作流，零額外腳本。30 workflow tests + 5 API tests 通過。 | Self |
