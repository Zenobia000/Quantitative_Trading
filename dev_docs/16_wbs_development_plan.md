...
> **當前狀態**：整體約 86%（990/1144h）、**1126 tests pass / coverage ~92.6%**、CI 三 job 上線（後端 pytest+coverage gate / 前端 vitest / OpenAPI+DDL drift hard gate）。審判庭 truth gate 五缺陷已修（[ADR-030](./adrs/ADR-030-truth-gate-judgement-fix.md)）；模擬器成本吞噬 bug 已修（PR #148：rebalance 段重疊去重吞掉 lump 成本 → `trim_overlap`，panel 策略歷史數字曾屬免成本高估）。**inst_flow survivorship-clean 平台化重驗（ADR-032 工作流 + 423 檔含下市 + 真實成本）最終判決 REJECTED**（DSR 0.908 < 0.95；WFA OOS+ 100% / holdout 0.892 / K3 0.846 皆過——死於通縮顯著性，詳 [inst_flow_truth_gate_verdicts](./inst_flow_truth_gate_verdicts.md)）；paper-ready 撤回，下一步＝掃描新 edge family 或依 ADR-025 討論邊緣候選小倉位進 paper（需新決策）。研究工作流 doe/go_gates/truth_gate/paper_replay/build_universe 全走 ADR-028 dispatch，新策略只寫 `research_config.py`。ADR 至 032。
>
