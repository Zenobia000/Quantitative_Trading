...
| 8.H.7 | **M3.6 validation+promotion service（CRITICAL #3）**：抽 `promotion_service.py`、持久化 validation_status/stage + immutable promotion_audit、`/research/validate/*`·`/research/promote/*` | 16h | 🟡 | 2026-06-07 | **核心 ✅**：`promotion_service` + event-sourced `validation_store`/`promotion_store`（append-only = immutable audit）；`/validate/{id}/gate-state` 真接、`/promote/{id}` GET/POST 嚴格 draft→paper→live 順序閘 + audit（26 tests）。剩 `/validate/{id}/wfa`(S7)·`/redline`(PBO/DSR matrix) |
...
| 項目 | 當前值 | 目標值 |
|:--|:---:|:---:|
| 整體進度 | **68%**（與 §2 工作包統計合計一致；v0.2–v0.8 後端 waves 已合併，#48-#53 batch）| 100% |
| M1 完成度 | 100% | 100% |
