...
| **7.A.4** | **paper 前移：接「過真偽閘」候選收 live OOS（[ADR-025](./adrs/ADR-025-two-stage-validation-gate-and-paper-promotion.md)）** | — | (時間) | 🟡 | 2026-06-14 | **候選已定＝資金流 fixed-config**（`inst_flow_truth_gate.py` TRUTH GATE REAL）✅。**chain replay ✅（2026-06-14）：`strategies/inst_flow/signal_fn.py`（因子 rank→buy 訊號，重用 `flow_intensity`，gate-safe sizing）+ `scripts/inst_flow_paper_replay.py` 把候選接 7.D real collaborators 跑完整 ETL→signals→risk→orders→log over 歷史資料 → 🟢 CHAIN GREEN：7 真訊號全過 12 條風控、PaperBroker 成交、寫真實 TimescaleDB（signals=7/orders=7/equity=1）。5 單元測試**。剩：**forward 即時 paper 收 live OOS**（needs daemon〔8.H.8〕+ **真實日曆時間**）。paper 摩擦回饋配置閘（8.G.10）|
...
| 8.H.8 | **M4 live-telemetry daemon（CRITICAL #1, needs-data）**：`runtime/` paper daemon、實作 `upsert_signals/orders/fills`、`market_reader`、全 `/monitor/*`、editable alerts | 24h | 🟡 | 2026-06-14 | **replay 核心 ✅（2026-06-14）：`runtime/paper_daemon.py`（`run_paper_replay` 逐日跑 chain、跨日 resilient、`replay_schedule` 重用策略 rebalance 排程）+ 6 單元測試 + `scripts/inst_flow_daemon_replay.py` 實跑 2023 四季 over 真實資料 + shared broker → 🟢 4/4 session green、DB telemetry signals=31/equity_snapshots=4（equity 演進 10M→10.13M）。`upsert_signals/orders/fills` 已實作+整合測（7.A.2）**。剩：`market_reader`（live feed）+ forward 即時排程（**真實時間**）+ 全 `/monitor/*` endpoints + editable alerts。解鎖 mon_a/b/c/d；WS `/ws/positions/live` 仍 M5 |
