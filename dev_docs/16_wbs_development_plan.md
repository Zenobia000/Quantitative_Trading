...
| 8.G 研究工作流（後端+前端）| 94h | 63h | 67% | ✅ gate_state/gate_machine + trials→DSR + CLI 全套 + runs DDL（8.G.1）+ research 前端 9 頁 shipped（含 Promote/TradeReview）；剩 8.G.6 token 擴充 + Cmd-K + **8.G.9 gate 拆兩段 + 8.G.10 配置閘 sizing（ADR-025）**。**✅ gate-state 權威已統一（refactor/gate-state-authority，2026-06-10）：`gate_machine` 集中單一 IS 推導（`derive_is_state`/`_is_state_from_result`）+ 詞彙橋（`IS_VERDICT_TO_STATUS` 寫 / `coerce_gate_state` 讀，含 round-trip 不漂移測），cli `_resolve_validation_status` 改用 `coerce_gate_state` 不再靜默忽略持久化 status，修 promote-check 讀小寫 verdict bug**|
...
| **合計** | **1144h** | **909h** | **79%** | M1 ✅ + v0.2–v0.8 後端 waves ✅ + **M3 研究迴圈後端**（PR #76-#78）+ **模組 6.0 統計驗證收尾（6.2.2/6.5.x 結，DOE 方法學執行到結構性定論：4 family 同 ~0.9 Sharpe 牆、無可部署 edge）** + **8.H.1 `Envelope[T]` typed responses（PR #98-100）+ 7.A.2/7.D paper-trade 持久化 & real collaborators（PR #101/#102）**。剩餘缺口主要為 needs-data/實盤（7.B/8.H.8）+ **驗證閘兩段化（8.G.9/8.G.10，ADR-025）+ paper 前移（7.A.4）** + 前端重頁（脫離 binary edge-gate，ADR-025）+ 策略 GO（外部約束 gated）|
...
| **7.A.4** | **paper 前移：接「過真偽閘」候選收 live OOS（[ADR-025](./adrs/ADR-025-two-stage-validation-gate-and-paper-promotion.md)）** | — | (時間) | ⏳ | — | 首個候選＝資金流 fixed-config（survivorship-clean WFA OOS 1.30、pre-registered）。前置：8.G.9 真偽閘判定 + 該 config 過真偽閘；接 7.D real collaborators（7.D.3 已備）。paper 報告回饋配置閘（8.G.10）|
...
| **8.G.9** | **`gate_machine` 拆兩段（[ADR-025](./adrs/ADR-025-two-stage-validation-gate-and-paper-promotion.md)）**：`TruthGate`（survivorship-clean + PBO/單 config OOS/DSR/K3，binary hard-fail）+ `SizingGate`（Sharpe/相關性/容量 → 倉位，連續；絕對 CAGR 降參考）；pre-registered 單 config 走 OOS+DSR 判真偽（不被 landscape PBO 連坐）| DEV | 10h | ⏳ | — | 純函式可 TDD；不破壞既有 `ValidationGate` 介面（新增非取代）|
| **8.G.10** | **配置閘 sizing 函數 spec + 實作**：倉位 = f(OOS Sharpe, 與艦隊相關性, capacity)；疊加 24 風控 spec；對接 ADR-022 艦隊配置 | DEV | 8h | ⏳ | — | 依賴 8.G.9 |

**模組小計**：~94h（已完成 ~63h ≈ 67%）| 後端契約 ~80%（8.G.0 對標 + 8.G.3 gate_state/gate_machine + 8.G.4 trials→DSR + 8.G.5 CLI 全套〔run-is/runs/sweep/compare/validate/promote-check〕✅；剩 8.G.2 RunConfig OOS/sweep 鎖死 + 8.G.1 runs 主表 DDL + **8.G.9 gate 拆兩段 + 8.G.10 配置閘 sizing（ADR-025）**）；**前端 8.G.7 research 工作區 7 頁已 shipped（platform-first pivot 期間建置，非延後）**，剩 8.G.6 token 擴充 + 8.G.8 Promotion stepper + Cmd-K
