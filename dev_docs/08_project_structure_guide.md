...
> **v1.7 變更（2026-07-03, ADR-041）**：**新增** `research/branch_store.py`（分支實驗血統簿 `branch_experiments.jsonl` — create/evaluate/compare，parent 記錄零改動）+ `api/routers/research_branches.py`（5 端點：branches CRUD-lite + evaluate + compare）。`research/simulation.py` 的 branch_suggestion 由 `simulation` fork 接上（Goal 9）。CLI 加 `branches` 子命令組。
