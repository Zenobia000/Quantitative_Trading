...
> 由 `global/02_backtest_platform_brand_system.md`（壓縮 Tokens）+ `pages/02_panel_b_positions.md` 組裝。
...
最高準則聲明：**dark-first、teal 主色、flat 1px border 無陰影、Geist Mono 數值、漲跌雙編碼、即時數據無動畫。任何輸出不得偏離上述 Tokens。**

---

## === CURRENT TASK: BUILD 面板 B — 部位狀態 (Positions) ===

本次任務：依上方 Global Guideline，實作 backtest_platform M3 的「部位狀態」儀表板頁面（route `/dashboard/positions`）。資料來源：positions + universe + daily_bars。這是 React 版（自 Streamlit 升級）：`st.metric` → KPI Card、`st.dataframe` → DataTable、Plotly 圖 → Recharts/Plotly.js。

需實作 5 個 Section（細節摘要，完整規格見 `pages/02_panel_b_positions.md`）：
...
4. **industry_allocation**（go.Pie / Recharts Pie）：產業別市值佔比，色盤用 dataviz 序列；hover 顯示金額(NT$)+佔比；點扇區 → cross-filter positions_table。
...
2. **一致性說明**：簡述如何落實設計一致性 — 配色僅取自上方 Tokens、數值一律 Geist Mono tabular-nums 右對齊、漲跌與 Heat 警示採色+文字雙編碼、flat 1px border 無陰影、即時刷新無進場動畫、focus-visible ring 用 accent #22D3EE。
...
*組裝日期: 2026-06-01 | 使用 backtest_platform Design System (dark-first) | 面板 B — M3 Positions*
