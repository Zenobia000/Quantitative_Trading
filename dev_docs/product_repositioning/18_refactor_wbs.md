...
- **本波 baseline**：`POSTGRES_PASSWORD=change_me_in_production uv run pytest -o addopts=""`（本機 codex pg 需此前綴強制 placeholder，否則「無 DB 降級」單元測試假失敗）→ 1457 passed / 3 skipped / 0 failed（另 1 pre-existing 失敗 test_ingest_universe_resolves_symbols 屬 codex WP10 symbols_for/data_root，非本波）；lint-imports 3 kept；drift All contract checks passed。**多 session**：W5.1~W6.1 全累積於 branch `refactor/w5.1-execution`，因 codex 反覆 checkout main，逐波 CAS 推 main 會在其 dirty worktree 腳下移 ref → services/ 顯示為 staged deletion，codex 一 commit 即 revert。故待 main 確定不 checked out 才一次 FF 落地（W5.1+W5.2a/b 已落地 `7652577`，其餘待窗口）。
