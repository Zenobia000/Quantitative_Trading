...
| ID | 波次 | 工作 | 依賴 | 分支策略 | 規模 | 本 session |
| :-- | :-- | :--- | :-- | :-- | :-- | :-: |
| W0.1 | 0-cleanup | 刪除 `backtest_platform/legacy/`（archive、不打包、不收集、0 引用） | — | feature-branch | S | ✅ |
| W0.2 | 0-cleanup | 刪死碼與 stale bytecode：`engines/`、`adapters/data_bundle/__init__.py`、influx_writer.pyc、`tests/engines/`、frontend `stores|utils/.gitkeep` | — | feature-branch | S | ✅ |
| W1.1 | 1-fitness | 加 import-linter，契約：research/strategies/validation **禁 import** adapters.brokers / risk / orchestration / monitoring；domain 禁 import sqlalchemy/fastapi/shioaji/requests。接 CI。 | W0.2 | feature-branch | M | ✅ |
...
| W2.1 | 2-governance | 抽第 3 層出 research/：`promotion_*`、`watch_registry`、`live_oos_*` → 新 `governance/` package；留 re-export shim 保住 daemon | W1.1 | worktree | L | ⏳ 下一波 |
...
| W3.1 | 3-purify | 拆 `strategies/inst_flow/signal_fn.py`：純 flow_intensity ranking 留 research；qty-sizing/stop_loss/priority 移 strategy_runtime/risk_gate | W1.1 | feature-branch | M | ⏳ |
