...
> 版本: v1.2 | 日期: 2026-07-06 | 狀態: **全 8 波次完成**（W0–W7 + backend 徹底碎解進 golden）。`backtest_platform` package 已消失，backend 分散進 `quant_platform/{apps/api, packages/{domain,application,adapters,infrastructure,contracts}, services/8}`，import root=`quant_platform`。
>
...
| W7.1 | 7-monorepo | 建 `quant_platform/{apps,packages,services}`；frontend→apps/web_console；deploy/ 與 research-note md 進 docs/ | W6.1 | worktree | XL | ✅ done（見 §3.1）：趁 codex quiet window，frontend→`quant_platform/apps/web_console`、backend 整包→`quant_platform/backtest_platform`、contracts→`quant_platform/packages/contracts`、research-note→`quant_platform/docs/research_notes`。**維持 backtest_platform import path**（810 imports/pyproject/import-linter/uvicorn 零改），只重接 7 config 錨點。1453 passed / lint 3 kept / drift OK。內部 package 碎解（→packages/services 新 import root）留 M1-M6 逐服務波次 |
...
- **W7.1 物理搬移完成（2026-07-06，趁 codex quiet window）**：偵測到 codex 停止（無 `--yolo` process、三 worktree 皆乾淨、最近 commit 全是本 session）→ quiet repo 條件成立，執行 big-bang。架構分析（`architect...w7.1`）確認**唯一正解＝維持 `backtest_platform` package 名、只搬目錄、零 import 改寫**（810 imports 橫跨 276 檔；改名+內部碎解成 `packages/`+`services/` 新 import root 是 M1-M6 逐服務工程，非 W7.1；scaffold README 映射表列的是「最終目標」不是「W7.1 一次到位」）。4 個獨立可綠 commit 同一 PR：C3 frontend→`apps/web_console`、C1 contracts→`packages/contracts`、C4 backend 整包→`quant_platform/backtest_platform`、C2 research-note→`docs/research_notes`。**維持 package 名的紅利**：pyproject（packages.find/pythonpath/coverage/import-linter root_package）、uvicorn `backtest_platform.api.app:app`、deploy `-m backtest_platform...` 模組路徑、既有 4 個過渡 shim **全零改**，唯一實質工作是重接 7 個路徑錨點（drift BACKEND_DIR/OPENAPI_SNAPSHOT、CI ×5 working-directory、deploy WorkingDirectory/EnvironmentFile/ExecStart、test_check_openapi_drift parents 層數）。test_profiles.py contracts 路徑因 backend+contracts 同進 quant_platform/ 而 parents[4] 自動對齊、零改。驗證：1453 passed / 3 skipped / 1 failed（唯一 test_ingest_universe_resolves_symbols 屬 codex WP10 pre-existing）、lint 3 kept、drift All contract checks passed、真 daemon 模組路徑不變。**內部 package 碎解（M1-M6）**：把 `src/backtest_platform/{research/domain,services/risk_gate,...}` 真正拆成 `quant_platform.{packages,services}.*` 新 import root，屆時舊路徑留 re-export shim——那才是 scaffold `quant_platform/services/*` 空目錄的填充波次。
- **backend 徹底碎解進 golden 完成（2026-07-06，使用者拍板 A+ONE-SHOT，架構分析 `architect-2026-07-06-0917-*`）**：不保留 `backtest_platform` package，內容碎解進 golden。**packaging = 單一 `quant_platform` package + src-layout**（`src/quant_platform/{apps,packages,services}`，`where=["src"]` 不變；不做 multi-package workspace＝過度設計）。**命名 A**：import root=`quant_platform`、研究核心=`services/research_validation/`（遵 golden，非 research_platform）、`research_platform` 只當 pyproject `[project].name` 品牌名。執行＝**有序映射表 longest-prefix-first 一次原子 remap**（498 imports；research 三向 domain/application/adapters→packages、其餘→services/research_validation；data 分裂 infrastructure vs data_platform；db_writer/run_persist aggregate shim 按 symbol 拆導）+ 刪 44 個 shim（risk/monitoring/jobs/orchestration/adapters.brokers/runtime.paper_daemon/data.db_writer + 19 research flat shims）+ config 全重接（pyproject 上移 quant_platform/、import-linter 3 契約全改、uvicorn `quant_platform.apps.api.app:app`、deploy `-m quant_platform.services.strategy_runtime.cli`、drift 腳本、seed script；**環境變數名不改**＝不破 userspace）。**刻意不加 packages⊄services 契約**（packages.application 合法 import services.research_validation.runners 為 strategy 註冊 seam，既有耦合；加則破 gate）。**6 gate 獨立複驗全綠**：rg backtest_platform in-scope=0、lint 3 kept/0 broken、compileall+collection 0 ImportError、pytest **1454 passed/3 skipped/0 failed**（cov 92.87%）、drift All contract checks passed、domain 純度 KEPT。單一原子 commit（405 檔，+1205/-2064）。ADR-R07 記錄 one-shot supersede R05 per-service 粒度（review 單位＝映射表+gate）。**後續**：前端 web_console mock/註解仍指舊 `backtest_platform.*`（前端 scope，另處理）；`quant_platform/README` 遷移表述為未完成計畫、實已 done（doc-sync 後續）。
